...
class ConnectionScenariosExtension implements TestTemplateInvocationContextProvider {
  ...
  private void executeTimeout(TestStep<?> step) {
    ScenarioMockUtil.testTimeout(step.getBuilder(), step::executeStep);
  }

  private void executeConnectionReset(TestStep<?> step) {
    ScenarioMockUtil.testConnectionReset(step.getBuilder(), step.executeStep());
  }

  private void executeFirstRequestFails(TestStep<?> step) {
    var firstStep = ScenarioMockUtil.firstRequestFails(step::getBuilder, step.executeStep());
  ...
  private void executeFirstAndSecondRequestsFail(TestStep<?> step) {
    var firstStep =
        ScenarioMockUtil.firstAndSecondRequestsFail(step::getBuilder, step.executeStep());
    verify(step, firstStep);
  }
