          ...
          .onErrorResume(
              serializationError ->
                  fetchCapabilityStatement(gicsClient)
                      .flatMap(
                          capabilityStatement -> {
                            // Ok, the server speaks FHIR, responded with an error status code that
                            // we would expect from gics,
                            // but did not return a valid OperationOutcome...
                            var outcome = new OperationOutcome();
                            outcome.addIssue().setDiagnostics("very strange indeed");
                            return Mono.just(outcome);
                          })
                      .onErrorResume(
                          e -> {
                            log.error("Unexpected error connecting to gICS", e);
                            return Mono.error(
                                new NoFhirServerException("Unexpected error connecting to gICS"));
                          }))
          .flatMap(
              b -> {
