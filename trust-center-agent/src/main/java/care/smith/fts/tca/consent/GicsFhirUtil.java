...
import care.smith.fts.tca.consent.configuration.NoGicsException;
...
public interface GicsFhirUtil {
  ...
  static Mono<CapabilityStatement> verifyGicsCapabilities(WebClient gicsClient) {
    return FhirClientUtils.fetchCapabilityStatement(gicsClient)
        .flatMap(
            c1 ->
                c1.getRest().getFirst().getResourceFirstRep().getProfile().contains("gics")
                    ? Mono.just(c1)
