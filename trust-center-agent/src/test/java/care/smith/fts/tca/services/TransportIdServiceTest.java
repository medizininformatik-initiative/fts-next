...
class TransportIdServiceTest {
  ...
  void storeAllMappingsStoresInRedis() {
    ...
    var result = service.storeAllMappings("transfer-1", allData, ttl);
  ...
  void storeMappingStoresDirectlyInRedis() {
    when(bucket.set(eq("sId-1"), eq(ttl))).thenReturn(Mono.empty());

    var result = service.storeMapping("tId-1", "sId-1");
  ...
  void storeMappingsStoresMultipleMappings() {
    ...
    var result = service.storeMappings(mappings, ttl);
  ...
  void storeMappingsWithEmptyMapCompletesImmediately() {
    var result = service.storeMappings(Map.of(), ttl);
