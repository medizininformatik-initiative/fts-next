...
function download_file() {
  curl -sSfL --retry 5 --retry-all-errors --retry-delay 1 --retry-max-time 30 \
    -u "${share}:" "${base_url}/kds/${1}.json.gz" -o "${data_dir}/${1}.json.gz"
}
...
{ echo "hospitalInformation";
  ...
  curl -sSLf --retry 5 --retry-all-errors --retry-delay 1 --retry-max-time 30 \
    -u "${share}:" "${base_url}/authored.json" | jq -rc "to_entries | .[0:${2:-100}] | .[].key"
