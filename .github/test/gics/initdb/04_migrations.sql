...
UPDATE sequence
SET SEQ_COUNT = (SELECT COALESCE(MAX(ID), 0) FROM virtual_person)
WHERE SEQ_NAME = 'virtual_person_index'
  AND SEQ_COUNT < (SELECT COALESCE(MAX(ID), 0) FROM virtual_person);
