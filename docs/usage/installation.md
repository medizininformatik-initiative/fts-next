...
For each agent, we offer an archive containing the necessary files and directories for running using
...
This process helps protect users from tampered or malicious builds by using cryptographic proofs of
provenance. Each agent release includes metadata that allows users to independently verify the
origin and integrity of both the archive files and container images.
