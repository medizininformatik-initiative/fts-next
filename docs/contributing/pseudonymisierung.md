...
Nachdem der RDA das transportpseudonymisierte Bundle erhalten hat, fordert dieseer vom TCA die
...
ChatGPT sagt: Jetzt mit den Längen 12, 16 und 24 – die Zeiten werden für große Alphabetgrößen
schnell astronomisch. 😃

### HMAC-SHA256

::: info
Es gibt derzeit keine Pläne HMAC-SHA256 zu implementieren.
:::

Um die Sicherheit der Pseudonymisierung weiter zu erhöhen, kann HMAC-SHA256 (Hash-based Message
Authentication Code mit SHA-256) verwendet werden.
Dieses Verfahren kombiniert eine kryptografische Hashfunktion (SHA-256) mit einem geheimen
Schlüssel, um eine Authentifizierung und Integritätsprüfung zu gewährleisten.

In der Pseudonymisierung kann HMAC-SHA256 verwendet werden, um sicherzustellen, dass die generierten
Pseudonyme nicht durch eine einfache Hash-Invertierung zurückgerechnet werden können.

Dies erschwert Brute-Force-Angriffe erheblich, da der geheime Schlüssel nicht bekannt ist und somit
keine Rainbow-Tables oder Preimage-Angriffe möglich sind.

**Sicherheitsvorteile von HMAC-SHA256:**

- Schutz gegen Kollisionsangriffe: SHA-256 ist resistent gegen Kollisionen, und durch die
  HMAC-Kombination mit einem geheimen Schlüssel wird dieser Schutz verstärkt.

- Schutz gegen Rainbow-Table-Angriffe: Da der Schlüssel geheim ist, können Angreifer keine
  vorgefertigten Tabellen verwenden.

- Schutz gegen Timing-Angriffe: HMAC-SHA256 hat eine konstante Laufzeit, was das Erraten des
  Schlüssels durch Timing-Analysen erschwert.
