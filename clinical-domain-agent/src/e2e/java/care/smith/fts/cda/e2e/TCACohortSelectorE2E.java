...
public class TCACohortSelectorE2E {
  ...
  private void configureTcaMocks() throws IOException {
    ...
    var classLoader = ClassLoader.getSystemClassLoader();
    var tcaResponseStream = classLoader.getResourceAsStream("tca-response.json");
    var tcaResponse = new String(tcaResponseStream.readAllBytes());

    tcaWireMock.register(
        post(urlPathMatching("/api/v2/cd/consented-patients.*"))
            .withHeader(CONTENT_TYPE, equalTo(APPLICATION_JSON_VALUE))
            .willReturn(jsonResponse(tcaResponse)));
